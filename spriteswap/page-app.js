...
async function tradePage(){
  ...
  const currentOwnedIds=new Set([...ownedIds].filter(id=>currentTradeIds.has(id)));
  ...
  let offer=Array.isArray(draft.offer)?draft.offer.filter(n=>byName(n)&&currentOwnedIds.has(byName(n).id)).slice(0,8):[];
  ...
  if(preOffer&&byName(preOffer)&&currentOwnedIds.has(byName(preOffer).id)&&!offer.includes(byName(preOffer).name))offer.unshift(byName(preOffer).name);
  ...
  const allOffersOwned=()=>offer.every(n=>{const sp=byName(n);return sp&&currentOwnedIds.has(sp.id)});
  ...
  const renderPicker=()=>{
    ...
    let rows=currentTradeSprites().filter(sp=>pickerSide==='offer'?currentOwnedIds.has(sp.id):!sp.unreleased);
