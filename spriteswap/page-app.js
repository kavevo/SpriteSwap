...
async function rateTradeDialog({peerId,tradeId,traderName='this trader',force=false}={}){
  if(!sb||!peerId||!tradeId)return false;
  const session=await ses(); if(!session)return false;
  const key=`spriteswap-rating-prompt:${tradeId}`;
  const existing=await sb.from('ratings').select('id,stars,comment').eq('rater_id',session.user.id).eq('rated_id',peerId).eq('trade_id',tradeId).maybeSingle();
  if(existing.data)return false;
  if(!force&&sessionStorage.getItem(key)==='shown')return false;
  sessionStorage.setItem(key,'shown');
  return new Promise(resolve=>{
    let d=document.getElementById('rateTradeDialog');
    if(!d){
      d=document.createElement('dialog');d.id='rateTradeDialog';d.className='rate-trade-dialog';
      d.innerHTML=`<form class="rate-trade-card">
        <button type="button" class="rate-trade-close" aria-label="Close">×</button>
        <div class="rate-trade-kicker">TRADE COMPLETE</div>
        <div class="rate-trade-icon">★</div>
        <h3>How was the trade?</h3>
        <p>Rate <b id="rateTradeName"></b>. Your review helps other collectors know who is reliable.</p>
        <div class="rate-star-row" role="radiogroup" aria-label="Rating out of 5">
          ${[1,2,3,4,5].map(n=>`<button type="button" class="rate-star active" data-rate-star="${n}" aria-label="${n} star${n===1?'':'s'}">★</button>`).join('')}
        </div>
        <input id="rateTradeStars" type="hidden" value="5">
        <label class="rate-trade-comment">Comment <span>optional</span><textarea id="rateTradeComment" maxlength="160" placeholder="Fast, fair trade…"></textarea></label>
        <p id="rateTradeNote" class="form-note"></p>
        <div class="rate-trade-actions"><button type="button" class="btn btn-secondary" id="rateTradeLater">Maybe later</button><button class="btn btn-primary" type="submit">Submit rating</button></div>
      </form>`;
      document.body.appendChild(d);
      d.querySelector('.rate-trade-close').onclick=()=>d.close('later');
      d.querySelector('#rateTradeLater').onclick=()=>d.close('later');
      d.querySelectorAll('[data-rate-star]').forEach(btn=>btn.onclick=()=>{
        const stars=+btn.dataset.rateStar;d.querySelector('#rateTradeStars').value=stars;
        d.querySelectorAll('[data-rate-star]').forEach(x=>x.classList.toggle('active',+x.dataset.rateStar<=stars));
      });
    }
    d.querySelector('#rateTradeName').textContent=traderName;
    d.querySelector('#rateTradeStars').value='5';d.querySelector('#rateTradeComment').value='';d.querySelector('#rateTradeNote').textContent='';
    d.querySelectorAll('[data-rate-star]').forEach(x=>x.classList.add('active'));
    const form=d.querySelector('form');
    form.onsubmit=async e=>{
      e.preventDefault();
      const button=form.querySelector('button[type="submit"]');button.disabled=true;
      const payload={rater_id:session.user.id,rated_id:peerId,trade_id:tradeId,stars:+d.querySelector('#rateTradeStars').value,comment:d.querySelector('#rateTradeComment').value.trim()};
      const r=await sb.from('ratings').upsert(payload,{onConflict:'rater_id,rated_id,trade_id'});
      button.disabled=false;
      if(r.error){d.querySelector('#rateTradeNote').textContent=r.error.message;return}
      pageToast('Rating saved — thanks for trading fairly');sessionStorage.setItem(key,'rated');d.close('rated');
    };
    const done=()=>{d.removeEventListener('close',done);resolve(d.returnValue==='rated')};d.addEventListener('close',done);d.showModal();
  });
}
...
async function profilePage(){
  ...
  $('#pageRatingForm')?.addEventListener('submit',async e=>{e.preventDefault();const f=new FormData(e.target);const {error}=await sb.from('ratings').upsert({rater_id:s.user.id,rated_id:id,trade_id:P.get('trade')||null,stars:+f.get('stars'),comment:f.get('comment')},{onConflict:'rater_id,rated_id,trade_id'});$('#pageRatingNote').textContent=error?error.message:'Rating saved.';if(!error)setTimeout(()=>location.href=`/profile.html?id=${encodeURIComponent(id)}`,500)});
}
...
async function tradePage(){
  ...
  const isSeason4=sp=>{const x=String(sp?.season||'').toLowerCase().replace(/\s+/g,' ').trim();return x==='c7 s4'||x.includes('chapter 7 season 4')||x.includes('chapter 7 s4')||x==='season 4'||x==='s4'};
  const season4Sprites=()=>sprites.filter(sp=>isSeason4(sp)&&!sp.unreleased);
  const chapter7Season4Ids=new Set(season4Sprites().map(sp=>sp.id));
  ...
  const renderPicker=()=>{
    ...
    let rows=(pickerSide==='offer'?season4Sprites():currentTradeSprites()).filter(sp=>pickerSide==='offer'?!sp.unreleased:!sp.unreleased);
    ...
    $('#tradePickerGrid').innerHTML=rows.length?rows.map(sp=>{const chosen=selected.includes(sp.name),blocked=other.includes(sp.name),full=selected.length>=8&&!chosen,owned=offerableOwnedIds.has(sp.id),needsVault=pickerSide==='offer'&&!owned;return `<button type="button" class="trade-picker-card ${variantClass(sp.variant)} ${chosen?'selected':''} ${needsVault?'not-owned':''} ${blocked||full?'disabled':''}" data-pick-name="${esc(sp.name)}" data-sprite-id="${esc(sp.id)}" ${blocked||full?'disabled':''}>${variantBadge(sp)}${needsVault?'<span class="vault-lock-badge">NOT IN VAULT</span>':''}<span class="trade-picker-art"><span class="variant-fx"></span><img src="${esc(sp.image)}" data-fallback="${esc(baseArt(sp))}" onerror="if(this.dataset.fallback){this.src=this.dataset.fallback;this.dataset.fallback=''}else this.src='assets/spriteswap-sprite-logo-128.png'" alt=""></span><b>${esc(sp.name)}</b><small>${blocked?'Already on other side':chosen?'Selected':needsVault?'Tap to mark owned + offer':`${esc(sp.variant)} · ${esc(sp.rarity.toUpperCase())}`}</small><i>${chosen?'✓':needsVault?'＋ VAULT':'+'}</i></button>`}).join(''):'<div class="empty">No Season 4 Sprites match these filters.</div>';
  };
  const openPicker=side=>{pickerSide=side;$('#spritePickerPanel').classList.remove('hidden');$('#pickerTitle').textContent=side==='offer'?"ADD SPRITES YOU'RE OFFERING":"ADD SPRITES YOU WANT";$('#pickerHelp').innerHTML=side==='offer'?`Season 4 only · <b>${offerableOwnedIds.size}</b> already in your vault out of <b>${season4Sprites().length}</b>. <span class="current-season-note">Unowned cards stay visible so you can mark them Owned and offer them.</span>`:'Current/new released Sprites only. <span class="current-season-note">Chapter 7 Season 3 is hidden.</span> Select up to 8.';$('#tradePickerSearch').value='';$('#tradePickerVariant').value='all';$('#tradePickerRarity').value='all';renderPicker();$('#spritePickerPanel').scrollIntoView({behavior:'smooth',block:'center'})};

  document.addEventListener('click',async e=>{const op=e.target.closest('[data-open-picker]');if(op){openPicker(op.dataset.openPicker);return}const rm=e.target.closest('[data-remove-name]');if(rm){const arr=rm.dataset.removeSide==='offer'?offer:want,idx=arr.indexOf(rm.dataset.removeName);if(idx>=0)arr.splice(idx,1);renderZones();return}const pc=e.target.closest('[data-pick-name]');if(pc&&!pc.disabled){const arr=sideArr(pickerSide),n=pc.dataset.pickName,sp=byName(n),idx=arr.indexOf(n);if(idx>=0){arr.splice(idx,1);renderZones();return}if(arr.length>=8||otherArr(pickerSide).includes(n))return;if(pickerSide==='offer'&&sp&&!offerableOwnedIds.has(sp.id)){pc.disabled=true;pc.classList.add('saving');const {error}=await sb.from('sprite_inventory').upsert({user_id:s.user.id,sprite_id:sp.id,owned:true,mastered:false,rarity:sp.rarity||'rare'},{onConflict:'user_id,sprite_id'});pc.classList.remove('saving');if(error){pc.disabled=false;$('#formNote').textContent=`Could not add ${sp.name} to your vault: ${error.message}`;return}ownedIds.add(sp.id);offerableOwnedIds.add(sp.id);$('#formNote').textContent=`${sp.name} added to your vault and trade.`}arr.push(n);renderZones()}});
...
async function tradeDetailPage(){
  ...
  if(!session) actions='<a class="btn btn-primary" href="/api/discord-login">Sign in with Discord to request</a>';
    ...
    else if(accepted?.status==='completed') actions=`<a class="btn btn-secondary" href="/messages.html?peer=${encodeURIComponent(accepted.requester_id)}&trade=${encodeURIComponent(t.id)}">View trade chat</a><button class="btn btn-primary" id="rateTradeBtn">Rate trader</button>`;
    else actions='<a class="btn btn-secondary" href="/#trades">Back to trade board</a>';
  }else{
    if(accepted&&accepted.requester_id===uid) actions=accepted.status==='completed'?`<a class="btn btn-secondary" href="/messages.html?peer=${encodeURIComponent(t.user_id)}&trade=${encodeURIComponent(t.id)}">View trade chat</a><button class="btn btn-primary" id="rateTradeBtn">Rate trader</button>`:`<a class="btn btn-primary" href="/messages.html?peer=${encodeURIComponent(t.user_id)}&trade=${encodeURIComponent(t.id)}">Open trade chat</a><button class="btn btn-secondary" id="completeTradeBtn">${viewerCompleted?'✓ You confirmed':`Mark completed (${completeCount}/2)`}</button>`;
  ...
  $('#completeTradeBtn')?.addEventListener('click',async()=>{if(!accepted||viewerCompleted)return;const r=await sb.rpc('confirm_trade_complete',{p_request_id:accepted.id});if(r.error)return pageToast(r.error.message,true);if(r.data){pageToast('Trade completed — both traders confirmed');await rateTradeDialog({peerId:isMine?accepted.requester_id:accepted.owner_id,tradeId:t.id,traderName:name(acceptedPeer),force:true});location.reload()}else{pageToast('Marked complete — waiting for the other trader');setTimeout(()=>location.reload(),550)}});
  $('#rateTradeBtn')?.addEventListener('click',()=>rateTradeDialog({peerId:isMine?accepted?.requester_id:t.user_id,tradeId:t.id,traderName:name(acceptedPeer||t.trader),force:true}));
...
async function messagesPage(){
  ...
  function render(){
    ...
    $('#pageThread').innerHTML=`<div class="accepted-system-message ${st==='closed'?'completed':''}"><span>${st==='closed'?'✓':'↔'}</span><div><b>${st==='closed'?'Trade completed':'Trade accepted'}</b><small>${st==='closed'?'Both traders confirmed the swap. This conversation is now read-only.':`${fmt(req.accepted_at||req.updated_at||req.created_at)} · Coordinate the in-game swap here. Never share passwords or login codes.`}</small></div></div>${active.rows.length?active.rows.map(m=>`<div class="msg-row ${m.sender_id===uid?'mine':''}"><div class="msg ${m.sender_id===uid?'mine':''}">${esc(m.body)}<small>${fmt(m.created_at)}</small></div></div>`).join(''):(st==='closed'?'<div class="message-empty-state compact"><strong>Trade completed</strong><span>There were no chat messages before completion.</span></div>':'<div class="message-empty-state compact"><strong>Chat is unlocked</strong><span>Send the first message to coordinate when you’re both available.</span></div>')}${st==='closed'?`<div class="completed-chat-notice"><span>✓</span><div><b>Trade was completed</b><small>Messaging is closed for this trade. You can still view the history, report an issue, or rate ${esc(name(active.p))}.</small></div><button id="messageRateTrader" class="btn btn-primary btn-small">Rate trader</button></div>`:''}`;
    $('#pageThread').scrollTop=$('#pageThread').scrollHeight;
    if(t){const os=tradeItems(t,'offer'),ws=tradeItems(t,'want'),ownerView=t.user_id===uid;$('#messagesTrade').innerHTML=`<div class="trade-side-head"><div><strong>Trade details</strong><small>#${esc(t.id.slice(0,10))}</small></div><span class="conversation-status ${st}">${st==='active'?'ACCEPTED':'COMPLETED'}</span></div><div class="accepted-chat-side"><label>${ownerView?'YOU OFFER':'THEY OFFER'} · ${os.length}</label><div class="accepted-chat-grid">${os.map(n=>spriteTile(n,'OFFER')).join('')}</div></div><div class="trade-swap-arrow">⇅</div><div class="accepted-chat-side"><label>${ownerView?'YOU WANT':'THEY WANT'} · ${ws.length}</label><div class="accepted-chat-grid">${ws.map(n=>spriteTile(n,'WANT')).join('')}</div></div><div class="trade-description"><label>DESCRIPTION</label><p>${esc(t.note||'No description provided.')}</p></div><div class="trade-completion-card"><div><b>Trade completion</b><span>${confirmations}/2 traders confirmed</span></div><div class="completion-track"><i style="width:${confirmations*50}%"></i></div>${st==='active'?`<button id="messageCompleteTrade" class="trade-side-btn complete" ${viewerDone?'disabled':''}>${viewerDone?'✓ You confirmed':'✓ Mark completed'}</button>`:'<div class="trade-completed-stamp">✓ Both traders confirmed</div>'}</div><div class="trade-side-actions">${st==='closed'?'<button id="messageRateTraderSide" class="trade-side-btn rate">★ Rate trader</button>':''}<a class="trade-side-btn" href="/trade.html?id=${encodeURIComponent(t.id)}">View full trade</a><a class="trade-side-btn" href="/profile.html?id=${encodeURIComponent(active.peer)}">View trader profile</a><a class="trade-side-btn danger" href="/report.html?user=${encodeURIComponent(active.peer)}&trade=${encodeURIComponent(t.id)}">Report trader</a></div>`;$('#messageCompleteTrade')?.addEventListener('click',async()=>{const r=await sb.rpc('confirm_trade_complete',{p_request_id:req.id});if(r.error)return pageToast(r.error.message,true);if(r.data){pageToast('Trade completed — both traders confirmed');await rateTradeDialog({peerId:active.peer,tradeId:t.id,traderName:name(active.p),force:true});location.reload()}else{pageToast('Marked complete — waiting for the other trader');setTimeout(()=>location.reload(),550)}})}
    const composer=$('#pageMessageForm'),input=$('#pageMessageInput'),send=composer?.querySelector('button');
    const closed=st==='closed';if(input){input.disabled=closed;input.placeholder=closed?'Trade completed — messaging is closed':'Write a message…'}if(send)send.disabled=closed;composer?.classList.toggle('is-closed',closed);
    $('#messageRateTrader')?.addEventListener('click',()=>rateTradeDialog({peerId:active.peer,tradeId:active.trade,traderName:name(active.p),force:true}));
    $('#messageRateTraderSide')?.addEventListener('click',()=>rateTradeDialog({peerId:active.peer,tradeId:active.trade,traderName:name(active.p),force:true}));
    if(closed)setTimeout(()=>rateTradeDialog({peerId:active.peer,tradeId:active.trade,traderName:name(active.p)}),120);
  }
