...
## V25 picker fix
- Offering remains Chapter 7 Season 4 only.
- All released Season 4 Sprites are visible in the picker even if they are not yet in your vault.
- Owned Season 4 Sprites can be selected immediately.
- An unowned Season 4 Sprite can be marked Owned and added to the trade in one click.
- This prevents an empty offering picker when a user has not indexed the new season yet.


## V26 — completion + ratings polish

- When both traders confirm completion, SpriteSwap opens an in-site rating dialog.
- Completed chats become read-only and the composer clearly says why messaging is closed.
- Rating aggregates can update safely without tripping the protected-profile trigger.
- Ratings are stored per completed trade.
- The accepted-trade detail rail is larger and easier to scan.

If V22/V25 is already installed, run `V26-RATING-CHAT-MIGRATION.sql` once in Supabase SQL Editor.
