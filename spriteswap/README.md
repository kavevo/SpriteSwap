...
Pre-release variant-accuracy and collection polish build.
...
## V16 live catalog
V16 can discover newly listed Sprite/variant pages from Fortnite.GG. Run the latest `schema.sql` once so the `sprite_catalog` and `sprite_sync_state` tables exist. The browser merges server-discovered rows into the static baseline automatically.

Live trade cards also show relative post age and collection-aware match badges.

## V18 profile + Sprite details
V18 increases public profile collection card size so Sprite art is fully visible, adds profile collection filters/share, and adds a dedicated `sprite.html` details page with live trades and vault actions. No new environment variables or SQL changes are required if V16/V17 is already installed.
