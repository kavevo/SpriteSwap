(() => {
  ...
  function renderTrades(){
    ...
    const rows=state.trades.filter(t=>{ const hay=`${t.offer} ${t.want} ${t.note} ${t.profiles?.username||''}`.toLowerCase(); return (!q||hay.includes(q))&&(tier==='all'||t.tier===tier); });
    ...
    $('#tradeGrid').innerHTML=rows.length?rows.map(t=>{
      ...
      return `<article class="trade-card ${perfect?'perfect-match':''}">${perfect?'<div class="match-ribbon">⚡ PERFECT MATCH</div>':''}<div class="trade-top"><button class="user-link" data-profile="${t.user_id}">${t.profiles?.avatar_url?`<img class="inline-user-avatar" src="${esc(t.profiles.avatar_url)}" alt="">`:''}${esc(t.profiles?.discord_name||t.profiles?.username||'collector')}</button><span class="tier">${esc(t.tier)}</span></div><div class="rating">★ ${Number(t.profiles?.rating_avg||0).toFixed(1)} <span class="muted">(${t.profiles?.rating_count||0})</span></div><div class="trade-pair"><div><span>Offering</span><strong>${esc(t.offer)}</strong></div><b>→</b><div><span>Wants</span><strong>${esc(t.want)}</strong></div></div>${perfect?`<div class="match-callout">YOU OWN ${esc(t.want).toUpperCase()} · MESSAGE THEM NOW</div>`:''}${t.note?`<p class="trade-note">${esc(t.note)}</p>`:''}<div class="meta">${fmt(t.created_at)}</div><div class="trade-actions">${state.session?.user.id!==t.user_id?`<button class="btn btn-primary btn-small" data-message="${t.user_id}" data-trade="${t.id}">Message</button><button class="btn btn-secondary btn-small" data-rate="${t.user_id}" data-trade="${t.id}">Rate</button><button class="btn btn-secondary btn-small" data-report-user="${t.user_id}" data-report-trade="${t.id}">Report</button>`:`<button class="btn btn-secondary btn-small" data-close-trade="${t.id}">Close trade</button>`}</div></article>`;
