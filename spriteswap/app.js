(() => {
  ...
  const state = { session:null, me:null, trades:[], profiles:[], ratings:[], messages:[], messageTrades:[], inventories:[], activePeer:null, activeTradeId:null, messageFilter:'all', messageSearch:'', adminTab:'reports', collection: loadCollection() };
  ...
  async function refreshAll(){
    ...
    if(state.session){
      const {data}=await sb.from('profiles').select('*').eq('id',state.session.user.id).single(); state.me=data||null;
      await syncCollectionFromCloud(); await loadMessages();
    } else { state.me=null; state.messages=[]; }
    renderAll();
  }
  ...
  function ownedNameSet(uid){ return new Set(inventoryFor(uid).flatMap(r=>{const s=spriteById(r.sprite_id);return s?[norm(s.name),norm(s.base)]:[];})); }
  ...
  function matchFor(uid){
    ...
    const mine=ownedNameSet(state.session.user.id), wants=[...new Set(state.trades.filter(t=>t.user_id===uid).map(t=>norm(t.want)).filter(Boolean))];
  ...
  function tradeCardSprite(name,label){
    ...
    return `<div class="trade-sprite-box ${sp?esc(sp.rarity):''}"><span class="trade-sprite-label">${esc(label)}</span><div class="trade-sprite-art"><img src="${esc(image)}" alt="${esc(name)}" onerror="this.onerror=null;this.src='assets/spriteswap-sprite-logo-128.png'"></div><strong>${esc(name)}</strong>${sp?`<small>${esc(sp.variant)} · ${esc(sp.rarity).toUpperCase()}</small>`:''}</div>`;
  }
  function renderTrades(){
    const q=$('#tradeSearch').value.trim().toLowerCase(), tier=$('#tierFilter').value;
    const rows=state.trades.filter(t=>{ const hay=`${t.offer} ${t.want} ${t.note} ${t.profiles?.username||''} ${t.profiles?.discord_name||''}`.toLowerCase(); return (!q||hay.includes(q))&&(tier==='all'||t.tier===tier); });
    const mine=state.session?ownedNameSet(state.session.user.id):new Set();
    $('#tradeGrid').innerHTML=rows.length?rows.map(t=>{
      const otherUser=state.session?.user.id!==t.user_id;
      const needOffer=!!state.session && otherUser && !mine.has(norm(t.offer));
      const canGive=!!state.session && otherUser && mine.has(norm(t.want));
      const perfect=needOffer && canGive;
      const ageMs=Date.now()-new Date(t.created_at).getTime();
      const fresh=ageMs < 10*60*1000;
      const who=t.profiles?.discord_name||t.profiles?.username||'collector';
      const avatar=t.profiles?.avatar_url||'assets/spriteswap-sprite-logo-128.png';
      const intel=state.session&&otherUser?`<div class="trade-intel">${needOffer?'<span class="trade-intel-badge need">MISSING FROM YOUR VAULT</span>':'<span class="trade-intel-badge owned">ALREADY OWNED</span>'}${canGive?'<span class="trade-intel-badge can-give">YOU HAVE WHAT THEY WANT</span>':'<span class="trade-intel-badge missing-want">YOU DON’T OWN THEIR WANT</span>'}</div>`:'<div class="trade-intel"><span class="trade-intel-badge neutral">SIGN IN FOR VAULT MATCHING</span></div>';
      return `<article class="trade-card ${perfect?'perfect-match':''} ${fresh?'fresh-trade':''}">
        ${perfect?'<div class="match-ribbon">⚡ BEST MATCH FOR YOU</div>':''}
        <div class="trade-card-head"><button class="user-link trader-chip" data-profile="${t.user_id}"><img class="inline-user-avatar" src="${esc(avatar)}" alt="" onerror="this.onerror=null;this.src='assets/spriteswap-sprite-logo-128.png'"><span><b>${esc(who)}</b><small>★ ${Number(t.profiles?.rating_avg||0).toFixed(1)} · ${t.profiles?.rating_count||0} ratings</small></span></button><div class="trade-live-meta"><span class="live-dot"></span><b>LIVE</b><small data-live-age="${esc(t.created_at)}">${relativeAge(t.created_at)}</small></div></div>
        <div class="trade-card-exchange">${tradeCardSprite(t.offer,'THEY OFFER')}<div class="trade-exchange-icon">⇄</div>${tradeCardSprite(t.want,'THEY WANT')}</div>
        ${intel}
        ${perfect?`<div class="match-callout">You need ${esc(t.offer)} and you already own ${esc(t.want)}. This is an exact vault match.</div>`:''}
        <div class="trade-card-note"><span>⚡</span><p>${esc(t.note||'Straight swap — no description provided.')}</p></div>
        <div class="trade-card-foot"><small>Posted <span data-live-age="${esc(t.created_at)}">${relativeAge(t.created_at)}</span></small><div class="trade-actions">${otherUser?`<button class="btn btn-primary btn-small" data-message="${t.user_id}" data-trade="${t.id}">Message</button><button class="btn btn-secondary btn-small" data-rate="${t.user_id}" data-trade="${t.id}">Rate</button><button class="btn btn-secondary btn-small" data-report-user="${t.user_id}" data-report-trade="${t.id}">Report</button>`:`<button class="btn btn-secondary btn-small" data-close-trade="${t.id}">Close trade</button>`}</div></div>
      </article>`;
    }).join(''):`<div class="empty">No matching open trades.</div>`;
    refreshLiveAges();
  }
  ...
  function buildNotifications(){
    if(!state.session) return [];
    const uid=state.session.user.id;
    const unread=state.messages.filter(m=>m.recipient_id===uid&&!m.read_at).slice(-6).reverse().map(m=>({kind:'message',title:'New message',text:`${m.sender?.discord_name||m.sender?.username||'A collector'} sent you a message.`,peer:m.sender_id,trade:m.trade_id||''}));
    const mine=ownedNameSet(uid);
    const matches=state.trades.filter(t=>t.user_id!==uid&&mine.has(norm(t.want))).slice(0,6).map(t=>({kind:'match',title:'Trade match',text:`${t.profiles?.discord_name||t.profiles?.username||'A collector'} wants ${t.want}, which is in your vault.`,peer:t.user_id,trade:t.id}));
    return [...unread,...matches].slice(0,10);
  }
  function renderHeaderPanels(){
    ...
    if($('#notificationsPanel')) $('#notificationsPanel').innerHTML=`<div class="popover-head"><b>Notifications</b><span>${notes.length} new</span></div>${notes.length?notes.map(n=>`<button type="button" class="notification-item" data-message="${n.peer}" data-trade="${n.trade}"><i>${n.kind==='match'?'◎':'●'}</i><span><b>${esc(n.title)}</b><small>${esc(n.text)}</small></span></button>`).join(''):'<div class="popover-empty">You’re all caught up.</div>'}`;
  }
  ...
  function renderThread(){
    ...
    if(trade){ $('#messageTradeNotice').innerHTML=`Trade chat for <b>${esc(trade.offer)}</b> → <b>${esc(trade.want)}</b>. Coordinate the in-game swap here — never share passwords or login codes.`; $('#messageTradeNotice').classList.remove('hidden'); }
  ...
  function updateUnread(){ const uid=state.session?.user.id; const n=uid?state.messages.filter(m=>m.recipient_id===uid&&!m.read_at).length:0; $('#unreadBadge').textContent=n; $('#unreadBadge').classList.toggle('hidden',!n); }
  ...
  function subscribeRealtime(){ if(!sb)return; sb.channel('spriteswap-live').on('postgres_changes',{event:'*',schema:'public',table:'trades'},()=>refreshAll()).on('postgres_changes',{event:'*',schema:'public',table:'sprite_inventory'},()=>refreshAll()).on('postgres_changes',{event:'INSERT',schema:'public',table:'messages'},async payload=>{ const uid=state.session?.user.id; if(uid&&(payload.new.sender_id===uid||payload.new.recipient_id===uid)){await loadMessages(); if($('#messageDialog').open)renderMessages();}}).subscribe(); }
  ...
  async function renderAdmin(){ let html=''; if(state.adminTab==='reports'){ const {data}=await sb.from('reports').select('*, reporter:reporter_id(username), reported:reported_user_id(username), trade:trade_id(offer,want)').order('created_at',{ascending:false}).limit(100); html=(data||[]).map(r=>`<div class="admin-row"><div><b>${esc(r.reason)}</b> <span class="tier">${esc(r.status)}</span><div class="muted">Reporter @${esc(r.reporter?.username||'?')} · Target @${esc(r.reported?.username||'?')}</div><div>${esc(r.details||'No details')}</div></div><div class="admin-actions">${['reviewed','dismissed','actioned'].map(s=>`<button class="btn btn-secondary btn-small" data-report-status="${r.id}:${s}">${s}</button>`).join('')}</div></div>`).join('')||'<div class="empty">No reports.</div>'; }
    if(state.adminTab==='trades'){ const {data}=await sb.from('trades').select('*, profiles:user_id(username)').order('created_at',{ascending:false}).limit(100); html=(data||[]).map(t=>`<div class="admin-row"><div><b>@${esc(t.profiles?.username||'?')}</b> — ${esc(t.offer)} → ${esc(t.want)} <span class="tier">${esc(t.status)}</span></div><div class="admin-actions"><button class="btn btn-danger btn-small" data-remove-trade="${t.id}">Remove</button></div></div>`).join(''); }
  ...
  function bindDynamic(root=document){
    ...
    root.querySelectorAll('[data-close-trade]').forEach(b=>b.onclick=async()=>{if(confirm('Close this trade?')){await sb.from('trades').update({status:'closed'}).eq('id',b.dataset.closeTrade); await refreshAll();}});
  ...
  function bindUI(){
    ...
    renderSprites();
    $('#tradeSearch').oninput=renderTrades; $('#tierFilter').onchange=renderTrades; $('#memberSearch').oninput=renderMembers;
    ...
    document.addEventListener('click',e=>{ const p=e.target.closest('[data-profile]'); if(p)openProfile(p.dataset.profile); const m=e.target.closest('[data-message]'); if(m)startMessage(m.dataset.message,m.dataset.trade); const r=e.target.closest('[data-rate]'); if(r&&requireAuth())openProfile(r.dataset.rate,true,r.dataset.trade); const rp=e.target.closest('[data-report-user]'); if(rp)openReport(rp.dataset.reportUser,rp.dataset.reportTrade); const ct=e.target.closest('[data-close-trade]'); if(ct&&confirm('Close this trade?')) sb.from('trades').update({status:'closed'}).eq('id',ct.dataset.closeTrade).then(refreshAll); });
  }
