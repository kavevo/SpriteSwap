(() => {
  ...
  function tradeCardSprites(names,label){
    const list=(names||[]).filter(Boolean), shown=list.slice(0,3);
    return `<div class="trade-sprite-group"><span class="trade-sprite-group-label">${esc(label)} <b>${list.length}</b></span><div class="trade-sprite-group-grid">${shown.map(n=>{const sp=spriteForTradeName(n),image=sp?.image||'assets/spriteswap-sprite-logo-128.png';return `<a class="trade-mini-sprite ${sp?variantStyle(sp.variant):''}" href="${sp?`/sprite.html?id=${encodeURIComponent(sp.id)}`:'#'}" aria-label="View ${esc(n)}"><span class="variant-fx"></span><img src="${esc(image)}" alt="${esc(n)}" onerror="this.onerror=null;this.src='assets/spriteswap-sprite-logo-128.png'"><small>${esc(n)}</small></a>`}).join('')}${list.length>3?`<span class="trade-mini-more">+${list.length-3}</span>`:''}</div></div>`;
  }
  function renderTrades(){
    const q=$('#tradeSearch').value.trim().toLowerCase(), tier=$('#tierFilter').value;
    const rows=state.trades.filter(t=>{ const offer=tradeItems(t,'offer'),want=tradeItems(t,'want'),hay=`${offer.join(' ')} ${want.join(' ')} ${t.note} ${t.profiles?.username||''} ${t.profiles?.discord_name||''}`.toLowerCase(); return (!q||hay.includes(q))&&(tier==='all'||t.tier===tier); });
    ...
    $('#tradeGrid').innerHTML=rows.length?rows.map(t=>{
      ...
      const needOffer=missingOffers.length>0, canGive=giveable.length>0, perfect=needOffer&&canGive;
      ...
      const intel=state.session&&otherUser?`<div class="trade-intel">${needOffer?`<span class="trade-intel-badge need">${missingOffers.length} MISSING FROM YOUR VAULT</span>`:'<span class="trade-intel-badge owned">ALL OFFERS ALREADY OWNED</span>'}${canGive?`<span class="trade-intel-badge can-give">YOU HAVE ${giveable.length} THEY WANT</span>`:'<span class="trade-intel-badge missing-want">NONE OF THEIR WANTS IN YOUR VAULT</span>'}</div>`:'<div class="trade-intel"><span class="trade-intel-badge neutral">SIGN IN FOR VAULT MATCHING</span></div>';
      return `<article class="trade-card ${perfect?'perfect-match':''} ${fresh?'fresh-trade':''}">
        ...
        <div class="trade-card-exchange multi">${tradeCardSprites(offers,'THEY OFFER')}<div class="trade-exchange-icon">⇄</div>${tradeCardSprites(wants,'THEY WANT')}</div>
        ${intel}
        ${perfect?`<div class="match-callout">You are missing ${esc(missingOffers[0])}${missingOffers.length>1?` +${missingOffers.length-1}`:''}, and you already own ${esc(giveable[0])}${giveable.length>1?` +${giveable.length-1}`:''}. Strong vault match.</div>`:''}
  ...
  function buildNotifications(){
    ...
    const matches=state.trades.filter(t=>t.user_id!==uid&&tradeNamesNorm(t,'want').some(n=>mine.has(n))).slice(0,6).map(t=>{const wanted=tradeItems(t,'want').filter(n=>mine.has(norm(n)));return {kind:'match',title:'Trade match',text:`${t.profiles?.discord_name||t.profiles?.username||'A collector'} wants ${wanted[0]||tradeItems(t,'want')[0]}${wanted.length>1?` +${wanted.length-1}`:''}, which is in your vault.`,peer:t.user_id,trade:t.id}});
