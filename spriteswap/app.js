(() => {
  ...
  async function submitRating(e){ e.preventDefault(); if(!requireAuth())return; const f=new FormData(e.target); const payload={rater_id:state.session.user.id,rated_id:f.get('rated_id'),trade_id:f.get('trade_id')||null,stars:+f.get('stars'),comment:f.get('comment')}; const {error}=await sb.from('ratings').upsert(payload,{onConflict:'rater_id,rated_id,trade_id'}); if(error)return toast(error.message,true); toast('Rating saved'); $('#profileDialog').close(); await refreshAll(); }
